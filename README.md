...
- **Moves**: a combo played on the video's clock, one move per revolution of the car. Moves are named like combo letters and laid out like the stick: R, UR, U, UL, L, DL, D, DR, N (neutral, just the clock) in the middle, and UT (U-turn) below. Pick the clock (single, double or triple) and how the stick gets back in time (jump back, or catch up as the video does), then build a combo from the pad. It is recorded once with the same physics and played on a **timeline**: play, pause, scrub, step a tick or a revolution, or pick a move to go to it. For the moment under the playhead the panel shows the stick with where it freezes, the move's steps with the current one lit, and the car's revolution with where the stick was held (orange) and went back in time (blue). Each move is measured **before and after**: from where the car was as its revolution started, and from where it is as the next one starts, the plain clock is played on and the two headings are compared. A plot draws each move from its before to its after, so a chain shows as a path.
...
The **heading gauge** (top right) looks straight ahead from the start. A point's distance from the centre is its angle from ahead, and its direction is where it lies. It plots the heading (gold dot, with its trail), the nose (small dot) and the spin axis (white ring). Angles use the game's axes as the car starts: X ahead, Y right, Z up. Yaw turns toward +Y and pitch toward +Z; the small gizmo shows those axes as the camera sees them. In Moves it shows the same plot as the panel instead: centred on where the combo starts, zoomed once to fit the whole combo, with each move drawn from before to after.
...
| Moves timeline: play or pause, step a tick, step a revolution | | Space, ← →, Shift + ← → |
...
The moves come from a search over every freeze point, circle and length, each played and measured exactly as the page plays and measures a combo. To run it again after changing the physics, the clock or the measurement:
